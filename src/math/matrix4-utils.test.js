...
test("generating XYZ matrix", () => {
  ...
  expect(r).toEqual("cy * cz, -(cy * sz), sy, 0, sx * sy * cz+cx * sz, cx * cz-(sx * sy * sz), -(sx * cy), 0, sx * sz-((cx * sy) * cz), (cx * sy) * sz+sx * cz, cx * cy, 0, 0, 0, 0, 1")
});

test("generating ZYX matrix", () => {
  const r = generateRotationMatrix([], rotZ, rotY, rotX).join(", ");
  expect(r).toEqual("cz * cy, cz * sy * sx-(sz * cx), sz * sx+cz * sy * cx, 0, sz * cy, cz * cx+sz * sy * sx, sz * sy * cx-(cz * sx), 0, -sy, cy * sx, cy * cx, 0, 0, 0, 0, 1");
});

test("generating ZYZ matrix", () => {
  const r = generateRotationMatrix([], rotZ, rotY, rotZ).join(", ");
  expect(r).toEqual("cz * cy * cz-(sz * sz), -(cz * cy * sz)-(sz * cz), cz * sy, 0, sz * cy * cz+cz * sz, cz * cz-(sz * cy * sz), sz * sy, 0, -(sy * cz), sy * sz, cy, 0, 0, 0, 0, 1");
});
