...
function parse(data, invertDirection) {
  ...
  function processToken(token) {
    switch(token.type) {
      ...
      case GEOMETRY_VERTEX:
        ...
        if (invertDirection) {
          currentGroup.vertices.push(v1, v3, v2);
        }
        else {
...
function getTriangleComponents(items, indexes) {
...
onmessage = (evt) => {
  ...
  .then(t => {
    timer.start();
    const obj = ObjFile.create(t, invertDirection);
